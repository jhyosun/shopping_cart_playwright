...
- 결함 리포트에서는 동일 결함을 다른 브라우저(Firefox, Safari, Edge)와 모바일 환경에서도 재현해보지 못한 한계가 존재
- 성능 지연(BUG-003)의 경우 "몇 초 걸린다"는 명확하지 않은 체감 수준으로 기술했는데, 정확하게 측정할 수 있는 툴을 이용해 실제 측정 수치(응답시간 ms, FCP, TTI 등)를 첨부하면 원인 파악에 더 빠르게 접근 할 수 있음
